...
class _BpProtocolScreenState extends ConsumerState<BpProtocolScreen> {
  ...
  Future<void> _startProtocol() async {
    ...
    if (diff < 6 * 60) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content:
                Text('La diferencia mínima entre sesiones debe ser de 6 horas'),
    ...
    if (!permissionGranted && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              'No recibirás recordatorios. Puedes continuar con el protocolo manualmente.'),
        ),
      );
    }
  ...
  Future<TimeOfDay?> _showTimePicker({required bool isMorning}) async {
    return showTimePicker(
      ...
      helpText:
          isMorning ? 'Hora de sesión matutina' : 'Hora de sesión nocturna',
    );
  }
  ...
  Future<void> _cancelProtocol() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            '¿Estás seguro de que deseas cancelar el protocolo activo?'),
        actions: [
          TextButton(
          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              title: Row(
                ...
                children: [
                  Image.asset('assets/logo/arteria-fit.png',
                      height: 28, width: 28),
                  const SizedBox(width: 8),
                  Text(
                    'Control 7 Días',
  ...
  Widget _buildProtocolContent() {
    ...
    final completedSessions =
        _sessions?.where((s) => s.isCompleted).length ?? 0;
  ...
  Widget _buildSessionButton(ThemeData theme, BpSession session) {
    ...
    return Padding(
      ...
      child: FilledButton.tonal(
        ...
        style: FilledButton.styleFrom(
          backgroundColor:
              session.isCompleted ? Colors.green.withValues(alpha: 0.1) : null,
